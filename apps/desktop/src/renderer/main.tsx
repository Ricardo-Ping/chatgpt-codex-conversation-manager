...
function ChatGptWorkspace({ bridge, state, onState, kind, onKind, onCounts }: { bridge: PairingState; state: ConversationState; onState(state: ConversationState): void; kind: "chat" | "work"; onKind(value: "chat" | "work"): void; onCounts(counts: Partial<Record<ConversationState, { chat: number; work: number }>>): void }) {
  ...
  async function moveProject(record: ManagedConversation, projectId: string | null): Promise<void> { try { await window.conversationManager.chatgpt.runBatch(accountKey, projectId ? "add-to-project" : "remove-from-project", [record.id], undefined, projectId ?? undefined); if (projectId) { const name = chatProjects.find((project) => project.id === projectId)?.name; if (name) setProjectNames((old) => ({ ...old, [projectId]: name })); } setRecords((old) => old.map((item) => item.id === record.id ? (projectId ? { ...item, projectId } : { ...item, projectId: undefined }) : item)); setNotice(projectId ? t("已添加到项目") : t("已移出项目")); void refreshCounts(); } catch (cause) { setError(friendlyError(cause)); } }
  async function moveProjectBatch(ids: string[], projectId: string | null): Promise<void> { try { await window.conversationManager.chatgpt.runBatch(accountKey, projectId ? "add-to-project" : "remove-from-project", ids, undefined, projectId ?? undefined); if (projectId) { const name = chatProjects.find((project) => project.id === projectId)?.name; if (name) setProjectNames((old) => ({ ...old, [projectId]: name })); } setRecords((old) => old.map((item) => ids.includes(item.id) ? (projectId ? { ...item, projectId } : { ...item, projectId: undefined }) : item)); setNotice(t("已移动 {n} 条会话", { n: ids.length })); void refreshCounts(); } catch (cause) { setError(friendlyError(cause)); } }
...
function CodexWorkspace({ onStatus, state, onState, onCounts }: { onStatus?(available: boolean): void; state: ConversationState; onState(state: ConversationState): void; onCounts(counts: Partial<Record<ConversationState, number>> | ((old: Partial<Record<ConversationState, number>>) => Partial<Record<ConversationState, number>>)): void }) {
  ...
  return <>
    ...
    }} projects={combinedCodexProjects} onReloadProjects={loadProjects} onProjectMove={moveProject} onBatchProjectMove={async (ids, projectId) => { try { for (const id of ids) await window.conversationManager.codex.setProject(id, projectId); const idSet = new Set(ids); const next = records.map((item) => idSet.has(item.id) ? (projectId ? { ...item, projectId } : { ...item, projectId: undefined }) : item); recordsByState.current = { ...recordsByState.current, [state]: next }; setRecords(next); setNotice(t("已移动 {n} 条会话", { n: ids.length })); } catch (cause) { setError(friendlyError(cause)); } }} onReadConversation={async (record) => { if (available !== true) throw new Error(t("暂时无法连接 Codex")); const data = await window.conversationManager.codex.read(record.id); return { title: record.title, messages: data.messages }; }} onDismissNotice={() => { setError(""); setNotice(""); }} />
    {dialog}
  </>;
}

function ManagerLayout(props: { source: "chatgpt" | "codex"; title: string; subtitle: string; emptyHint: string; kind?: "chat" | "work"; onKind?(value: "chat" | "work"): void; onOpenExternal?(): void; onExport?(ids: string[]): Promise<string>; accounts?: Account[]; accountKey?: string; onAccount?(key: string): void; state: ConversationState; onState(state: ConversationState): void; records: ManagedConversation[]; projectNames?: Record<string, string>; projects?: Array<{ id: string; name: string }>; projectsLoading?: boolean; onReloadProjects?(): void; onProjectMove?(record: ManagedConversation, projectId: string | null): void | Promise<void>; onReadConversation?(record: ManagedConversation): Promise<{ title: string; messages: Array<{ role: string; at: number | null; text: string }> }>; onBatchProjectMove?(ids: string[], projectId: string | null, records: ManagedConversation[]): Promise<void>; onDismissNotice?(): void; writable: boolean; refreshable: boolean; loading: boolean; error: string; notice: string; onRefresh(): void | Promise<void>; onOpen(record: ManagedConversation): void | Promise<void>; onCancel?(): Promise<{ cancelled: boolean }>; onBatch(action: "archive" | "restore" | "delete", ids: string[]): Promise<{ succeeded: string[]; failed: Array<{ id: string; message: string }>; unprocessed?: string[] } | null> }) {
  ...
  const [batchProjectOpen, setBatchProjectOpen] = useState(false);  const runBatchProjectMove = async (projectId: string | null) => { setBatchProjectOpen(false); if (busy || !props.onBatchProjectMove) return; setLocalNotice(""); setBusy(true); try { const ids = [...selected]; const records = props.records; await props.onBatchProjectMove(ids, projectId, records); setSelected(new Set()); } catch (cause) { setLocalNotice(`${t("操作失败")}：${message(cause)}`); } finally { setBusy(false); } };
  const openViewer = (record: ManagedConversation) => { if (!props.onReadConversation) return; if (viewer && viewer.record.id === record.id && (viewer.loading || (!viewer.error && viewer.messages.length))) return; setViewer({ record, title: record.title, messages: [], loading: true, error: "" }); void (async () => { try { const data = await props.onReadConversation!(record); setViewer((old) => old && old.record.id === record.id ? { ...old, title: data.title || old.title, messages: data.messages, loading: false } : old); } catch (cause) { setViewer((old) => old && old.record.id === record.id ? { ...old, loading: false, error: friendlyError(cause) } : old); } })(); };
    ...
    <div className="workspace-main">
      ...
      <div className="actionbar">
        ...
        <div className="selection-actions">
          <button type="button" className={allSelected ? "active" : ""} disabled={!selectable.length || busy} onClick={() => setSelected(new Set(selectable))}>{t("全选当前结果")}</button>
          <button type="button" disabled={!selected.size || busy} onClick={() => setSelected(new Set())}>{t("清空选择")}</button>
          {props.onBatchProjectMove && props.projects && props.projects.length > 0 && <>
            <button type="button" className={batchProjectOpen ? "active" : ""} disabled={!selected.size || busy} onClick={() => setBatchProjectOpen((old) => !old)}>{t("移动到项目")}</button>
            {batchProjectOpen && <div className="batch-project-menu">
              {props.projects.map((project) => <button type="button" key={project.id} onClick={() => void runBatchProjectMove(project.id)}>{project.name}</button>)}
              {[...selected].some((id) => props.records.find((record) => record.id === id)?.projectId) && <button type="button" onClick={() => void runBatchProjectMove(null)}>{t("移出项目")}</button>}
            </div>}
          </>}
        </div>
...
function Settings({ version, lang, onLanguage, onCodexStatus }: { version: string; lang: Lang; onLanguage(value: Lang): void; onCodexStatus?(available: boolean): void }) {
  ...
  return <section className="settings"><p className="eyebrow">Conversation Manager v{version}</p><h1>{t("设置与隐私")}</h1><div className="settings-grid">
    ...
    <article><h2>{t("Codex 项目")}</h2><p>{t("管理 Codex 的项目文件夹。")}</p>
      {codexProjects.length > 0 ? <div className="codex-project-list">{codexProjects.map((project) => (<div className="project-manage-row" key={project.id}><span className="project-manage-name">{project.name}</span><button type="button" className="project-manage-btn" onClick={() => { void window.conversationManager.codex.projectRename(project.id, prompt(t("重命名"), project.name) || project.name).then(reloadCodexProjects).catch(() => {}); }}>{t("重命名")}</button><button type="button" className="project-manage-btn" onClick={() => { void window.conversationManager.codex.projectDelete(project.id).then(reloadCodexProjects).catch(() => {}); }}>{t("删除")}</button></div>))}</div> : <p className="project-empty">{t("暂无项目")}</p>}
    </article>
