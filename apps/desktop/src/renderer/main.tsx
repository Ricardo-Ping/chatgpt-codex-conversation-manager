...
function Settings({ version, lang, onLanguage, onCodexStatus }: { version: string; lang: Lang; onLanguage(value: Lang): void; onCodexStatus?(available: boolean): void }) {
  ...
  return <section className="settings"><p className="eyebrow">Conversation Manager v{version}</p><h1>{t("设置与隐私")}</h1><div className="settings-grid">
    ...
    <article><h2>{t("Codex 项目")}</h2><p>{t("管理 Codex 的项目文件夹。")}</p>
      {codexProjects.length > 0 ? <div className="codex-project-list">{codexProjects.map((project) => (<div className="project-manage-row" key={project.id}><span className="project-manage-name">{project.name}</span><button type="button" className="project-manage-btn" onClick={() => { const name = window.prompt(t("重命名"), project.name); if (name && name.trim()) void window.conversationManager.codex.projectRename(project.id, name.trim()).then(reloadCodexProjects).catch(() => {}); }}>{t("重命名")}</button><button type="button" className="project-manage-btn" onClick={() => { void window.conversationManager.codex.projectDelete(project.id).then(reloadCodexProjects).catch(() => {}); }}>{t("删除")}</button></div>))}</div> : <p className="project-empty">{t("暂无项目")}</p>}
    </article>
