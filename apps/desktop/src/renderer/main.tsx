...
function App() {
  ...
  const [chatCounts, setChatCounts] = useState<Partial<Record<ConversationState, number>>>({});
...
function ChatGptWorkspace({ bridge, state, onState, onCounts }: { bridge: PairingState; state: ConversationState; onState(state: ConversationState): void; onCounts(counts: Partial<Record<ConversationState, number>>): void }) {
  const [accounts, setAccounts] = useState<Account[]>([]); const [accountKey, setAccountKey] = useState(""); const [kind, setKind] = useState<"chat" | "work">("chat");
...
function CodexWorkspace({ onStatus, state, onState }: { onStatus?(available: boolean): void; state: ConversationState; onState(state: ConversationState): void }) {
  ...
  const { confirm, dialog } = useConfirm();
  async function load(full = false) { setLoading(true); setError(""); try { const all: CodexThread[] = []; let cursor: string | null = null; do { const page = await window.conversationManager.codex.list({ cursor, archived, full }); all.push(...page.data); cursor = page.nextCursor; } while (cursor); const merged = [...new Map(all.map((thread) => [thread.id, toCodexManaged(thread, archived)])).values()]; setRecords(merged); setNotice(`${full ? "完整校准完成" : "同步完成"}：共 ${merged.length} 条任务`); } catch (cause) { setError(message(cause)); } finally { setLoading(false); } }
  ...
  useEffect(() => { if (available) void load(); }, [state]);
