...
export function Settings({ version, lang, onLanguage, onCodexStatus }: { version: string; lang: Lang; onLanguage(value: Lang): void; onCodexStatus?(available: boolean): void }) {
  ...
  useEffect(() => window.conversationManager.codex.onArchiveProgress((progress) => {
    setArchiveNotice(progress.phase === "collect" ? t("正在收集 Codex 会话…") : t("正在压缩 {n} 个会话…", { n: progress.files }));
  }), []);
  ...
  return <section className="settings"><p className="eyebrow">Conversation Manager v{version}</p><h1>{t("设置与隐私")}</h1><div className="settings-grid">
    ...
    <article><h2>{t("Codex 会话迁移")}</h2><p>{t("把本机全部 Codex 会话打包为 zip，在其他电脑导入后即可继续这些会话；不包含登录凭据。")}</p>{archiveNotice && <p className="pair-feedback">{archiveNotice}</p>}<div className="card-actions"><button onClick={async () => { try { const r = await window.conversationManager.codex.exportSessionsArchive(); setArchiveNotice(""); if (r.cancelled) return; setPairMessage(r.count ? t("已导出 {n} 个会话到 {file}", { n: r.count, file: r.file ?? "" }) : t("没有可导出的 Codex 会话")); } catch (cause) { setArchiveNotice(""); setPairMessage(friendlyError(cause)); } }}>{t("导出 Codex 会话")}</button><button onClick={async () => { try { const r = await window.conversationManager.codex.importSessionsArchive(); setArchiveNotice(""); if (r.cancelled) return; setPairMessage(t("已导入 {n} 个会话（跳过 {s} 个已存在）", { n: r.imported ?? 0, s: r.skipped ?? 0 })); } catch (cause) { setArchiveNotice(""); setPairMessage(friendlyError(cause)); } }}>{t("导入 Codex 会话")}</button></div></article>
    <article><h2>{t("托盘与快捷键")}</h2><p>{t("关闭主窗口后应用驻留系统托盘；随时按全局快捷键 Alt+Shift+Space 呼出快速搜索。")}</p></article>
