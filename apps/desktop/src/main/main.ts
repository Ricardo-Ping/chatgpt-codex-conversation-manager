...
ipcMain.handle("chatgpt:projects", async (event, value) => {
  ...
  const result = await bridge.request("projects", { accountKey }); if (!result.ok) throw new Error(result.error?.message || M().accountReadFailed);
  const payload = result.payload as { projects?: unknown }; const rows = Array.isArray(payload?.projects) ? payload.projects : [];
  return { projects: rows.slice(0, 300).map((row) => { const item = row && typeof row === "object" ? row as Record<string, unknown> : {}; return typeof item.id === "string" && item.id.startsWith("g-p-") && typeof item.name === "string" && item.name.trim() ? { id: item.id.slice(0, 128), name: item.name.trim().slice(0, 100) } : null; }).filter((row): row is { id: string; name: string } => Boolean(row)) };
});
...
ipcMain.handle("chatgpt:list", async (event, value) => {
  ...
  const result = await bridge.request("list", { accountKey, state, mode, checkpoint: mode === "full" ? null : cached?.records[0]?.updatedAt ?? null }, 180_000); if (!result.ok) throw new Error(result.error?.message || M().syncFailed);
